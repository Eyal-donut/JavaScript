...
function findFriends() {
  return data
    ...
    .filter((john) => john.friends.length > 0);
}
console.log("only with filter", findFriends());

// Exercise 2: Use the filter method to get all the people who live in New York
function findNewYorkers() {
  return data.filter((person) => person.address.city === "New York");
}
console.log(findNewYorkers());

// Exercise 3: Use the filter method to get all the people who are older than 30
const findOlderThan30 = () => data.filter((person) => person.age > 30);
console.log(findOlderThan30());
...
const getAddresses = () => data.map((person) => person.address);
...
const findFirstChicago = () =>
  data.find((person) => person.address.city === "Chicago");
console.log(findFirstChicago());

// Exercise 2: Use the find method to find the first person who is older than 30
const findFirstOlderThan30 = () => data.find((person) => person.age > 30);
console.log(findFirstOlderThan30());

// Exercise 3: Use the find method to find the first person who has "reading" as a hobby
const findFirstReader = () =>
  data.find((person) => person.hobbies.includes("reading"));
console.log(findFirstReader());
