...
const peopleNames = data.map(function (person){
  return person.name
})
...
const avgAge = ageArray.reduce((sum, current) => (sum + current), 0)/ageArray.length
console.log(avgAge)

// Question 4: Return an array with the unique cities where the people live.
const uniqCities = data.map((person) => person.address).map((addressObj) => { 
